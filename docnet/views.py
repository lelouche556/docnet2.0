...
from docnet.forms import LoginForm, AccountRegisterForm
from docnet.models import Account, Profile, Action, MedicalInfo
from docnet import logger
...
def authentication_check(request, required_roles=None, required_GET=None):
    ...
    if not request.user.is_authenticated:
        request.session['alert_danger'] = "You must be logged into docnet to view that page."
        return HttpResponseRedirect('/')
    # Sanity check. Users without accounts cannot interact with docnet
    try:
...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            request.session['alert_success'] = "Successfully logged into docnet."
            return HttpResponseRedirect('/profile/')
    else:
        form = LoginForm()
...
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            login(request, user)
            request.session['alert_success'] = "Successfully registered with docnet."
            return HttpResponseRedirect('/profile/')
    else:
...
def error_denied_view(request):
    ...
    # Proceed with the rest of the view
    return render(request, 'docnet/error/denied.html', template_data)